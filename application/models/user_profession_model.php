<?php
/**
* user profession fields Model
* 
*/

class User_profession_model extends MY_Model
{
    ...
    */
    public function user_profession_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    { $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("users u","u.id=up.uid","left");//left join
            $this->db->select('up.*,u.s_user_name');
            
            $rs=$this->db
                ->get_where("user_profession up",
                            array("up.id"=>$condition)
                )
                ->row();
        ...
        else
        {
            ...
            */
            $this->db->start_cache();
                ...
                $this->db->from("user_profession up");
            $this->db->stop_cache();
    ...
    */
    public function add_user_profession($values)
    {
        return $this->add_("user_profession",$values);
    }
    ...
    */
    public function update_user_profession($values,$where)
    {
        return $this->update_("user_profession",$values,$where);
    } 
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_profession($where)
    {
        return $this->delete_("user_profession",$where);
    }    
