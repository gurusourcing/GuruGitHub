<?php
/**
* user license fields Model
* 
*/

class User_license_model extends MY_Model
{
    ...
    */
    public function user_license_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    { $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("users u","u.id=ul.uid","left");//left join
            $this->db->select('ul.*,u.s_user_name');
            
            $rs=$this->db
                ->get_where("user_license ul",
                            array("ul.id"=>$condition)
                )
                ->row();
    ...
    */
    public function add_user_license($values)
    {
        return $this->add_("user_license",$values);
    }
    ...
    */
    public function update_user_license($values,$where)
    {
        return $this->update_("user_license",$values,$where);
    } 
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_license($where)
    {
        return $this->delete_("user_license",$where);
    }    
