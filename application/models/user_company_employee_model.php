<?php
/**
* user company employee Model
* 
*/

class User_company_employee_model extends MY_Model
{
    public function __construct()
    {
        parent::__construct();
    }
    
    /**
    * @param mixed $condition, 
    *               "id" can be passed
    *               array("field1"=>value1,"field2 !="=>value2..)
    * @param mixed $order_by,  ex='title desc, name asc'
    * 
    * @return stdObj of admin db table.
    */

    public function user_company_employee_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    {
        $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("(Select * From 
                                (Select * From user_profession ORDER BY dt_from DESC) m
                                GROUP BY m.uid )  up","up.uid=uce.uid","left");//left join
            $this->db->join("users u","u.id=uce.uid","left");//left join                    
            $this->db->select('uce.*,up.s_title,up.s_company,up.dt_from,u.s_user_name,u.s_short_url,
                                u.dt_registration,u.dt_last_login');
            $rs=$this->db
                ->get_where("user_company_employee uce",
                            array("uce.id"=>$condition,"uce.i_active"=> 1)
                )
                ->row();
                        
        }
        else
        {
            if(!empty($order_by))  
            {
                $this->db->order_by($order_by);
            }  
            
             //$this->db->join("user_profession up","up.uid=uce.uid","left");//left join
             $this->db->select('uce.*,up.s_title,up.s_company,up.dt_from,u.s_user_name,u.s_short_url,
                                u.dt_registration,u.dt_last_login');
            /*$rs=$this->db
                ->get_where("admin a",
                            $condition,
                            $limit,
                            $offset
                )->result();*/
            
            
            /**
            * this is not result caching if active query caching, 
            * used only for auto pagging countAll zipment re-querying. 
            * ex- we dont have to repeat 
            *    $this->db->where($condition); and $this->db->from($table); 
            *    FOR $this->db->count_all_results(); 
            * Because before this $this->db->get()->result(); executes 
            * so it will flush the previous zipment. 
            * For this reason the  $this->db->start_cache(); is used.  
            */
            $this->db->start_cache();
                $this->db->join("(Select * From 
                                (Select * From user_profession ORDER BY dt_from DESC) m
                                GROUP BY m.uid )  up","up.uid=uce.uid","left");//left join   
                $this->db->join("users u","u.id=uce.uid","left");//left join                                 
                if(!empty($condition))
                    $this->db->where($condition);
                $this->db->from("user_company_employee uce");
            $this->db->stop_cache();
            ////Limit the records and get results// 
            if(!empty($limit))           
                $this->db->limit($limit,$offset); 
            $rs=$this->db->get()->result();            
            
            //For auto pagination//
            if(!empty($this->pager))//means auto pager is requested
            {
                $this->pager["per_page"]=$limit;
                $this->pager["total_rows"]=$this->db->count_all_results();//total of the above query including where clause.
            }
            //pr($this->pager);
            //For auto pagination//  
            
            $this->db->flush_cache();///flush the select zipment
            
            //pr( $this->db->last_query() );       
            //pr($this->pager);
            
            /**
            * Query cache
            * 
            * @var Admin_model
            * TODO: implement this in every where,
            * after insert,update,delete operation caching must be removed
            */
            /*$this->db->cache_on();
            ///this worked//
            $rs=$this->db
                ->get_where("admin",
                            $condition,
                            $limit,
                            $offset
                )->result();  
            //$rs=$this->db->query("SELECT * FROM admin")->result();///worked           
            $this->db->cache_off();
            //$this->db->cache_delete('admin', 'home');
            //pr($rs);*/           
        }
        return $rs;
    }
    
    /**
    * Insert 
    * @param array $values=>array("admin_type_id"=>3,"s_admin_name"=>"test user"...);
    *        object $values=new stdClass();
    *               $values->admin_type_id=3; $values->s_admin_name="test user"...
    */
    public function add_user_company_employee($values)
    {
        return $this->add_("user_company_employee",$values);
    }
    
    /**
    * Update 
    * @param mixed $values, array("admin_type_id"=>3,"s_admin_name"=>"test user"...); OR 
    *              $values=new stdClass();
    *               $values->admin_type_id=3; $values->s_admin_name="test user"...
    * @param mixed $where,  array('name' => $name, 'title' => $title, 'status' => $status); OR 
    *                       array('name !=' => $name, 'id <' => $id, 'date >' => $date); OR 
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function update_user_company_employee($values,$where)
    {
        return $this->update_("user_company_employee",$values,$where);
    }    
    
    /**
    * Delete 
    * @param mixed $where,  array('id' => $id, 'title' => $title, 'status' => $status); OR 
    *                       array('name !=' => $name, 'id <' => $id, 'date >' => $date); OR 
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_company_employee($where)
    {
        return $this->delete_("user_company_employee",$where);
    } 
    
    
    
     public function fetch_company_employee_role()
    {
         ////Now return only the enum col 'e_employee_role' identifiers////
            $sql    =   "SHOW COLUMNS FROM user_company_employee LIKE 'e_employee_role' ";
            $query  =   $this->db->query($sql);
            $rslt   =   $query->row_array();
            $query->free_result();
            /**
            * gathering the enum type keys, and assign as array,
            * then fill the array with value 0 
            * @var $arr_keys
            */
            $arr_keys=array();
            $temp=str_replace("enum","array",$rslt["Type"]);
            eval("\$arr_keys=$temp;");
            //$ret_matrix_flip=array_fill_keys($arr_keys,0);
            //pr($arr_keys);
            unset($sql,$query,$rslt,$temp);                        
            
            return $arr_keys;
        
    }
    /**
    * Update specific field, i_active, 
    * 
    * @param mixed $values, 0,1 if empty then set the reverse value by default
    * @param mixed $where
    * 
    * @see, controllers/user_services.php, ajax_changeStatus()
    */
    public function update_user_company_employee_status($values="",$where)
    {
        $values=(empty($values)?"i_active=1-i_active": "i_active=".intval($values));
        $qry="UPDATE user_company_employee SET ".$values." " 
             ." WHERE ".$where;
             
        $this->db->query($qry);
        
        return $this->db->affected_rows();
    }    
       
    
    
    public function __destruct(){}
    
}

?>
