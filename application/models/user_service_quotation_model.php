<?php
/**
* user_service_quotation Model
* 
*/

class User_service_quotation_model extends MY_Model
{
    ...
    */
    public function user_service_quotation_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    {
        $rs=new stdClass();
              if(is_integer($condition))
              {
                $this->db->join("users u","u.id=usq.uid","left");//left join
                $this->db->join("users uby","uby.id=usq.uid_requested_by","left");//left join
                $this->db->select('u.s_user_name,usq.*,uby.s_user_name AS s_requested_by_user_name');
                $rs=$this->db
                ->get_where("user_service_quotation usq",
              ...
              else
              {
                  ...
                  */
                  $this->db->start_cache();
                    ...
                    $this->db->join("users uby","uby.id=usq.uid_requested_by","left");//left join
                    $this->db->select('u.s_user_name,usq.*,uby.s_user_name AS s_requested_by_user_name');                    
                    $this->db->from("user_service_quotation usq");
                  $this->db->stop_cache();
    ...
    */
    public function add_user_service_quotation($values)
    {
        return $this->add_("user_service_quotation",$values);
    }
    ...
    */
    public function update_user_service_quotation($values,$where)
    {
        return $this->update_("user_service_quotation",$values,$where);
    }    
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_service_quotation($where)
    {
        return $this->delete_("user_service_quotation",$where);
    }    
    
    /**
    * Fetching enum field values for creating dropdown
    */
    public function fetch_quotation_for_status()
    {
        ////Now return only the enum col 'e_doc_type' identifiers////
        $sql    =   "SHOW COLUMNS FROM user_service_quotation LIKE 'e_status' ";
