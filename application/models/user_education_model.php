...
class User_education_model extends MY_Model
{
    ...
    */
    public function user_education_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    { $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("users u","u.id=ue.uid","left");//left join
            $this->db->select('ue.*,u.s_user_name');
            
            $rs=$this->db
                ->get_where("user_education ue",
                            array("ue.id"=>$condition)
                )
                ->row();
        ...
        else
        {
            ...
            */
            $this->db->start_cache();
                ...
                $this->db->from("user_education ue");
            $this->db->stop_cache();
    ...
    */
    public function add_user_education($values)
    {
        return $this->add_("user_education",$values);
    }
    ...
    */
    public function update_user_education($values,$where)
    {
        return $this->update_("user_education",$values,$where);
    } 
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_education($where)
    {
        return $this->delete_("user_education",$where);
    }    
