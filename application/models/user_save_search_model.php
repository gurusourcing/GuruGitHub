<?php
/**
* user_save_search Model
* 
*/

class User_save_search_model extends MY_Model
{
    ...
    */
    public function user_save_search_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    {
        $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("users u","u.id=uss.uid","left");//left join
            $this->db->select('uss.*,u.s_user_name');
            
            $rs=$this->db
                ->get_where("user_save_search uss",
                            array("uss.id"=>$condition)
                )
                ->row();
            ...
            */
            $this->db->start_cache();
                if(!empty($condition))
                    ...
                    $this->db->join("users u","u.id=uss.uid","left");//left join
                    $this->db->select('uss.*,u.s_user_name');
                    $this->db->from("user_save_search uss");
            $this->db->stop_cache();
    ...
    */
    public function add_user_save_search($values)
    {
        return $this->add_("user_save_search",$values);
    }
    ...
    */
    public function update_user_save_search($values,$where)
    {
        return $this->update_("user_save_search",$values,$where);
    }    
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_save_search($where)
    {
        return $this->delete_("user_save_search",$where);
    }    
