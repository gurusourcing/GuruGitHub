<?php
/**
* user company license fields Model
* 
*/

class User_company_license_model extends MY_Model
{
    ...
    */
    public function user_company_license_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    { $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("user_company uc","uc.id=ucl.comp_id","left");//left join
            $this->db->select('ucl.*,uc.s_company');
            
            $rs=$this->db
                ->get_where("user_company_license ucl",
                            array("ucl.id"=>$condition)
                )
                ->row();
        ...
        else
        {
            ...
            */
            $this->db->start_cache();
                ...
                $this->db->from("user_company_license ucl");
            $this->db->stop_cache();
    ...
    */
    public function add_user_company_license($values)
    {
        return $this->add_("user_company_license",$values);
    }
    ...
    */
    public function update_user_company_license($values,$where)
    {
        return $this->update_("user_company_license",$values,$where);
    } 
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_company_license($where)
    {
        return $this->delete_("user_company_license",$where);
    }    
