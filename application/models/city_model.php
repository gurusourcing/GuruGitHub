...
class City_model extends MY_Model
{
    ...
    */
    ...
    public function city_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    {
        $rs=new stdClass();
        if(is_integer($condition))
        {
            $this->db->join("country cn","cn.id=cy.country_id","left");//left join
            $this->db->join("state s","s.id=cy.state_id","left");//left join
            $this->db->select('cy.*,cn.s_country,s.s_state');
            
            $rs=$this->db
                ->get_where("city cy",
                            array("cy.id"=>$condition)
                )
                ->row();
    ...
    */
    public function add_city($values)
    {
        return $this->add_("city",$values);
    }
    ...
    */
    public function update_city($values,$where)
    {
        return $this->update_("city",$values,$where);
    }    
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_city($where)
    {
        return $this->delete_("city",$where);
    }    
