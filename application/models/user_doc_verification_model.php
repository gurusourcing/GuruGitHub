<?php
/**
* user_doc_verification Model
* 
*/

class User_doc_verification_model extends MY_Model
{
    ...
    */
    public function user_doc_verification_load($condition,$limit=NULL,$offset=NULL,$order_by=NULL)
    {
        $rs=new stdClass();
              ...
              {
                  $this->db->join("users u","u.id=udv.uid","left");//left join
                  $this->db->join("doc_verification dv","dv.id=udv.doc_verify_id","left");//left join
                  $this->db->select('udv.*,u.s_user_name,dv.s_document_required');

                  $rs=$this->db
                      ->get_where("user_doc_verification udv",
                                  array("udv.id"=>$condition)
                      )
                      ->row();
       ...
       /*$rs=$this->db
                  ...
                  */
                  $this->db->start_cache();
                    ...
                    $this->db->from("user_doc_verification udv");
                  $this->db->stop_cache();
    ...
    *                       "name='Joe' AND status='boss' OR status='active'";
    */
    public function delete_user_doc_verification($where)
    {
        return $this->delete_("user_doc_verification",$where);
    }    
