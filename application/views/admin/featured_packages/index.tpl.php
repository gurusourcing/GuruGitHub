...
$(document).ready(function(){
    ...
    $('input[id="i_active"]').each(function(){
        $(this).change(function(){
            $.post('<?=admin_base_url('featured_packages/ajax_changeStatus')?>',
            {"id":{ 0: $(this).attr("value") }},
            function(data){
                window.location.href='<?= admin_base_url('featured_packages')?>';
           });
        });
    })
...
<div class="tablePars">
    <div class="dataTables_filter" id="DataTables_Table_0_filter">
        ...
        <label>Package : <input type="text" id="s_package_name" name="s_package_name" value="<?=@$posted["s_package_name"];?>"  aria-controls="DataTables_Table_0">
