...
switch($m.props.animType){case"none":newPos=$m.slideAnimSetup($m.props.sWidth,"left",pos);$m.props.sdiv.css({"left":newPos+"px"});$m.props.direct=null;$m.props.animReady=true;break;case"slide":newPos=$m.slideAnimSetup($m.props.sWidth,"left",pos);$m.props.sdiv.animate({"left":newPos+"px"},$m.props.animTime,$m.props.easingStyle,$m.transAnimCallback);break;case"slide-vertical":newPos=$m.slideAnimSetup($m.props.sHeight,"top",pos);$m.props.sdiv.animate({"top":newPos+"px"},$m.props.animTime,$m.props.easingStyle,$m.transAnimCallback);break;case"fade":var $current=$m.props.sdiv.find("> div:eq("+$m.props.currentSlide+")");var $prev=$m.props.sdiv.find("> div:eq("+$m.props.prevSlide+")");$current.css("z-index","100").animate({"opacity":"1"},$m.props.animTime,$m.transAnimCallback);$prev.css("z-index","10");if($m.props.prevSlide!==null){setTimeout(function(){$prev.css("opacity","0");},$m.props.animTime);}
break;default:}}};this.slideAnimSetup=function(dim,dir,pos){var output;var minus=dir=="top"?0:1;if($m.props.animInfinity){if($m.props.direct){$m.resetAllSlidePositions();$m.props.sdiv.css(dir,-(dim*$m.props.oldPos)+"px");output=-(dim*$m.props.currentSlide);}else{output=-dim+(dim*-$m.props.direction);}}else if($m.props.alwaysNext){$(".s-id-"+$m.props.oldPos,$m.props.sdiv).remove().prependTo($m.props.sdiv);$(".s-id-"+$m.props.currentSlide,$m.props.sdiv).remove().insertAfter($(".s-id-"+$m.props.oldPos,$m.props.sdiv));var listItem=$(".s-id-"+$m.props.oldPos,$m.props.sdiv);$m.props.sdiv.css(dir,"0px");output=($m.props.sdiv.css(dir).split("px")[0]-dim);}
else{output=(dim*-pos);}
...
if($m.props.direction==1){var firstSlide=$(".slide:first",$m.props.sdiv);firstSlide.remove().appendTo($m.props.sdiv);}
else if($m.props.direction==-1){var lastSlide=$(".slide:last",$m.props.sdiv);lastSlide.remove().prependTo($m.props.sdiv);}
if($m.props.animType=="slide"){$m.props.sdiv.css("left",-$m.props.sWidth+"px");}
if($m.props.animType=="slide-vertical"){$m.props.sdiv.css("top",-$m.props.sHeight+"px");}}
$m.props.direct=null;$m.props.animReady=true;if($m.props.animQueue!==null){$m.gotoSlide($m.props.animQueue,true,null);}};this.setNavActive=function(id){$("li",$m.props.navContainer).removeClass("active");$("> li:eq("+id+")",$m.props.navContainer).addClass("active");};this.timerFunc=function(){$m.gotoSlide($m.props.currentSlide+1,false,1);};this.resetTimer=function(startNow){clearInterval($m.props.timer);$m.props.timer=setInterval($m.timerFunc,$m.props.slideTime+$m.props.animTime);if(startNow){$m.timerFunc();}};this.resetAllSlidePositions=function(){for(var i=0;i<$m.props.sCount;i++){$(".s-id-"+i,$m.props.sdiv).remove().appendTo($m.props.sdiv);}};this.setSlideOrder=function(focusOn){var firstSlide=(focusOn-1<0?$m.props.sCount-1:focusOn-1);for(var i=0;i<$m.props.sCount;i++){var thisSlide=firstSlide+i;if(thisSlide>=$m.props.sCount){thisSlide-=$m.props.sCount;}
$(".s-id-"+thisSlide,$m.props.sdiv).remove().appendTo($m.props.sdiv);}};this.buildBackNextLinks=function(){var backNextNav='<ul class="slideshow-back-next">';backNextNav+='<li class="back-button"><a href="javascript:;">Back</a></li>';backNextNav+='<li class="next-button"><a href="javascript:;">Next</a></li>';backNextNav+='</ul>';$($m.props.sContainer).append(backNextNav);};this.buildPauseButton=function(){var pauseButton='<p class="pause-button playing"><a href="javascript:;">Pause</a></p>';$($m.props.sContainer).append(pauseButton);};this.buildNav=function(){var $nav=$m.props.sdiv.after('<ul class="'+$m.props.navContainerTitle+'" />');var $slides=$m.props.sdiv.find("> li");var slideNavContent="";for(var i=0;i<$m.props.sCount;i++){var ref=$slides.attr("id");slideNavContent+='<li><a href="#'+ref+'">Slide '+(i+1)+'</a></li>';}
$("."+$m.props.navContainerTitle,$m.props.sContainer).append(slideNavContent);}
this.init=function(root){$m.props.sContainer=root;$($m.props.sliderHolder,$m.props.sContainer).wrap("<div class='slider-wrapper' />");$m.props.sWrapper=($(".slider-wrapper",root));$("> div",$m.props.sWrapper).addClass("s-active "+$m.props.animType);$m.props.timer=null;$m.props.prevSlide=null;$m.props.currentSlide=0;$m.props.backNextContainer=".slideshow-back-next";$m.props.pauseContainer=".pause-button";$m.props.sRef=$($(".CarouSlide")).index($m.props.sContainer);$m.props.animReady=true;$m.props.animState=!$m.props.autoAnim?"pause":"play";$m.props.sdiv=$(".s-active",$m.props.sWrapper);$m.props.sdiv.find("> div",$m.props.sWrapper).each(function(id){$(this).addClass("slide").addClass("s-id-"+id);});$m.props.sCount=$(".s-active > div",$m.props.sWrapper).size();$m.props.sWidth=$(".s-active > div",$m.props.sWrapper).outerWidth();$m.props.sHeight=$(".s-active > div",$m.props.sWrapper).outerHeight();$m.props.navContainerTitle=$m.props.navContainer.split(".")[1];if($m.props.hoverLinks==true){$m.props.animType="none"}
...
if($m.props.animInfinity){$(".slide:last",$m.props.sdiv).remove().prependTo($m.props.sdiv);}
if($m.props.showSlideNav){$("> div:first",$m.props.navContainer).addClass("active");}else{$($m.props.navContainer).hide();}
if($m.props.autoAnim&&$m.props.showPauseButton){$m.buildPauseButton();}
if($m.props.showBackNext){$m.buildBackNextLinks();}
switch($m.props.animType){case"fade":$("> div",$m.props.sdiv).css({"opacity":"0","position":"absolute","left":0,"top":0,"z-index":10});$("> li:first",$m.props.sdiv).css({"z-index":"100","opacity":"1"});break;case"none":$(".s-active",$m.props.sWrapper).width(($m.props.sWidth*$m.props.sCount)+"px");break;case"slide":if($m.props.animInfinity){$($m.props.sdiv).css("left",-$m.props.sWidth+"px");}
$(".s-active",$m.props.sWrapper).width(($m.props.sWidth*$m.props.sCount)+"px");break;case"slide-vertical":if($m.props.animInfinity){$($m.props.sdiv).css("top",-$m.props.sHeight+"px");}
$("> div",$m.props.sWrapper).height(($m.props.sHeight*$m.props.sCount)+"px");break;default:}
