...
<div class="panel_info facebook_fans nopad">
           <p class="name">Verification</p>
            <div class="facebook nodevider nopad no-bot-mar">
                <ul class="name_list">
					...
					<?php if(!empty($verification)) {
							foreach($verification as $val)
								{
					 ?>
					 <li class="short"><a href="javascript:void(0);"><?php echo $val->s_document_required ?> <?= ((bool)$val->i_verified)?'':' not '?> verified</a> <img src="<?=base_url(get_theme_path())."/";?>images/<?= ((bool)$val->i_verified)?'green-right.png':'cross2.png'?>" width="16" height="16" alt="cross" class="alignright" /></li>
                ...
                <?php /*?><a href="javascript:void(0);" class="short_grey_button">Add more</a><?php */?>
				<a href="<?php echo base_url().'add_verification' ?>" class="short_grey_button">Add more</a>
            </div>
        </div>